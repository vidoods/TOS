...
$page_title = $is_edit_mode ? $lang['edit_plan'] : $lang['create_new_plan'];
$submit_btn_text = $is_edit_mode ? $lang['update_plan'] : $lang['save_plan'];
            ...
            <form id="plan-form">
                ...
                <div class="form-section-grid">
                    <div class="form-group">
                        <label for="plan-pair" class="form-label"><?= $lang['pair'] ?></label>
                        <select class="select-field" id="plan-pair" name="pair_id" required>
                            <option value=""><?= $lang['loading_dots'] ?></option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="plan-bias" class="form-label"><?= $lang['narrative_bias'] ?></label>
                        <select class="select-field" id="plan-bias" name="bias" required>
                            <option value="Neutral" selected><?= $lang['neutral'] ?></option>
                ...
                <div class="form-section-grid">
                    <div class="form-group">
                        <label for="plan-type" class="form-label"><?= $lang['plan_type'] ?></label>
                    ...
                    <div class="form-group">
                        <label for="plan-date" class="form-label"><?= $lang['date'] ?></label>
                        <input type="date" class="input-field" id="plan-date" name="date" value="<?= $default_date ?>" required>
                    </div>
                </div>

                <div class="form-group">
                    <label for="plan-title" class="form-label"><?= $lang['plan_title_label'] ?></label>
                    <input type="text" class="input-field" id="plan-title" name="title"
                           placeholder="<?= $lang['plan_title_placeholder'] ?>" required>
                </div>
				
				<div class="form-group">
                    <label for="plan-note" class="form-label"><?= $lang['link_note'] ?></label>
                    <select class="select-field" id="plan-note" name="note_id">
                         <option value=""><?= $lang['no_note'] ?></option>
                    </select>
                </div>

                <h2 style="font-size: 16px; color: var(--text-secondary); margin-top: 40px; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px;"><?= $lang['timeframe_analysis'] ?></h2>
                ...
                <button type="button" class="btn btn-secondary" style="width: 100%; margin-top: 15px; padding: 15px; border-style: dashed;" onclick="addTimeframe()">
                    <span>+</span> <?= $lang['add_timeframe'] ?>
                </button>

                <div style="display: flex; gap: 15px; margin-top: 40px; padding-top: 20px; border-top: 1px solid var(--glass-border);">
                    <button type="button" class="btn btn-outline" style="flex-grow: 1;" onclick="window.history.back()"><?= $lang['cancel'] ?></button>
